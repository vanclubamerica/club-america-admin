...
import { renderThemeCss } from '../src/lib/publish/theme-css';
import {
  renderEventList,
  renderFooterAddress,
  renderFooterSocial,
...
async function main() {
  ...
  // Written with the SAME function the publish pipeline uses. A hand-written
  // placeholder here would differ by a few bytes and make the first publish
  // report a spurious change to theme.css.
  const themeCssPath = join(SITE_PATH, 'css', 'theme.css');
  const themeCss = renderThemeCss(null);
  if (!existsSync(themeCssPath) || readFileSync(themeCssPath, 'utf8') !== themeCss) {
    writeFileSync(themeCssPath, themeCss, 'utf8');
